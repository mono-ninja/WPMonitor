<script lang="ts">
    ...
    $: apiUrl = `https://api.wpmonitor.dev/wp-json/wpmonitor/v1/wp/plugin/${slug}/stats`;
    ...
    async function checkCachedData() {
        if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined') {
            const storedData = sessionStorage.getItem(`wp:plugin:${slug}_historical`);
            if (storedData) {
    ...
    async function fetchData() {
        try {
            ...
            historicalData = data;
            sessionStorage.setItem(`wp:plugin:${slug}_historical`, JSON.stringify(data));
            formatData();
        } catch (error) {
