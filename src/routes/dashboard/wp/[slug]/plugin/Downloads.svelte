<script lang="ts">
    ...
    $: apiUrl = `https://api.wpmonitor.dev/wp-json/wpmonitor/v1/wp/plugin/${slug}/downloads`;
    ...
    async function checkCachedData() {
        if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined') {
            const storedData = sessionStorage.getItem(`wp:plugin:${slug}_downloads`);
            if (storedData) {
    ...
    async function fetchData() {
        try {
            ...
            downloadsData = data;
            sessionStorage.setItem(`wp:plugin:${slug}_downloads`, JSON.stringify(data));
            generateChartData();
        } catch (error) {
