<script lang="ts">
    ...
    $: apiUrl = `https://api.wordpress.org/themes/info/1.2/?action=theme_information&slug=${slug}`;
    ...
    async function checkCachedData() {
        ...
        if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined') {
            const storedData = sessionStorage.getItem(`wp:theme:${slug}_info`);
            if (storedData) {
    ...
    async function fetchData() {
        try {
            ...
            if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined') {
                sessionStorage.setItem(`wp:theme:${slug}_info`, JSON.stringify(data));
            }
            transformData();
            isDataReady = true;
        } catch (error) {
    ...
    function transformData() {
        ...
        if (itemData && typeof itemData === 'object') {
            ...
            let updated = formatDateToISO(itemData.last_updated_time);
            let add = formatDateToISO(itemData.creation_time)
            checkUpdate(itemData.last_updated_time);
...
{#if isDataReady}
    ...
    <a href="https://wordpress.org/themes/{slug}/" target="_blank"
       class="inline-flex justify-between items-center py-1 px-1 pr-4 mb-7 text-sm text-gray-900 bg-gray-100 rounded-full hover:bg-gray-200">
        <span
                class:bg-primary-200={notice === 0}
                class:bg-yellow-200={notice === 1}
                class:bg-red-200={notice === 2}
                class="text-xs rounded-full px-2 py-1.5 mr-3">{transformedData['updated']}</span> <span class="text-sm font-medium">Theme Page</span>
