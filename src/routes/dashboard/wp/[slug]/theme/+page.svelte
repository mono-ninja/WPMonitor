...
<script lang="ts">
    ...
    function clearCache() {
        sessionStorage.removeItem(`wp:theme:${slug}_downloads`);
        sessionStorage.removeItem(`wp:theme:${slug}_historical`);
        sessionStorage.removeItem(`wp:theme:${slug}_keywords`);
        location.reload();
    }
