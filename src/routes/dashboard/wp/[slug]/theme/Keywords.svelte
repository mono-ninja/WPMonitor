<script lang="ts">
    ...
    $: apiUrl = `https://api.wordpress.org/themes/info/1.2/?action=query_themes`;
    ...
    async function checkCachedKeywords() {
        if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined') {
            const storedKeywords = sessionStorage.getItem(`wp:theme:${slug}_keywords`);
            if (storedKeywords) {
    ...
    async function fetchKeywordsData() {
        const keywordPromises = keywords.map(async (keyword) => {
            ...
            if (result.themes) {
                const index = result.themes.findIndex((theme) => theme.slug === slug);
                if (index !== -1) {
        ...
        sessionStorage.setItem(`wp:theme:${slug}_keywords`, JSON.stringify(keywordResults));
    }
</script>
