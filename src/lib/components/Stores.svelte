<script lang="ts">
    ...
    import {eddStore} from '$lib/stores/eddStore';
    import {onMount} from "svelte";

    let eddItems = {};
    let results: any = {}; // Initialize as an object
    let isLoading = true;
    ...
    async function checkCachedData() {
        if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined') {
            const storedData = sessionStorage.getItem(`edd:info`);
            if (storedData) {
    ...
    async function fetchAllData() {
        if (Object.keys(eddItems).length === 0) {
            return;
        }

        for (const [name, item] of Object.entries(eddItems)) {

            const sales = await fetchStats(`https://api.wpmonitor.dev/wp-json/wpmonitor/v1/edd/${item.site}/${item.key}/${item.token}`, 'sales');
            const earnings = await fetchStats(`https://api.wpmonitor.dev/wp-json/wpmonitor/v1/edd/${item.site}/${item.key}/${item.token}`, 'earnings');
        ...
        if (results && Object.keys(results).length > 0) {
            sessionStorage.setItem('edd:info', JSON.stringify(results));
        }

        isLoading = false;
    }

    async function fetchStats(site: string, type: string) {
        try {
            const response = await fetch(`${site}/${type}`);
            ...
            <div class="rounded-xl border bg-white shadow">
                ...
                <div class="p-6 pt-0">
                    <div class="text-2xl font-bold">
                        {#if data?.earnings?.current_month}
                            {(data.earnings.current_month).toLocaleString("en-US",
                                {style: "currency", currency: "USD"})}
                        {:else}
                            0
                        {/if}
                    </div>
                    <p class="text-xs text-muted-foreground">{(data.sales.current_month).toLocaleString()} sales</p>
                </div>
            </div>
